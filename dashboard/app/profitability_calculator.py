...
PROFITABILITY_DATA_DIR = os.environ.get('PROFITABILITY_DATA_DIR', '/var/lib/ephemery/data/lido-csm/profitability')
...
class ProfitabilityCalculator:
    ...
    def load_performance_history(self) -> pd.DataFrame:
        ...
        if not os.path.exists(history_file):
            ...
            df = pd.DataFrame(columns=[
                'timestamp', 'validator_count', 'attestation_rate', 'proposal_rate',
                'avg_balance', 'avg_rewards_daily', 'eth_price'
            ])
    ...
    def fetch_validator_performance(self) -> Dict[str, Any]:
        ...
        try:
            ...
            response = requests.get(f"{CSM_API_ENDPOINT}/api/v1/validators/performance", timeout=10)
            response.raise_for_status()
    ...
    def update_performance_history(self) -> None:
        ...
        self.performance_history = self.performance_history.drop_duplicates(subset=['timestamp']).sort_values('timestamp')
    ...
    def calculate_monthly_costs(self, validator_count: int = 1) -> Dict[str, float]:
        ...
        hardware_cost = self.cost_inputs.get('hardware_cost_monthly', 100.0) * validator_count
        
        # Variable costs (scale with validator count but not linearly)
        power_cost = self.cost_inputs.get('power_cost_monthly', 20.0) * (1 + (validator_count - 1) * 0.5)
        bandwidth_cost = self.cost_inputs.get('bandwidth_cost_monthly', 30.0) * (1 + (validator_count - 1) * 0.3)
        
        # Maintenance costs
        maintenance_hours = self.cost_inputs.get('maintenance_hours_monthly', 5.0) * (1 + (validator_count - 1) * 0.2)
        maintenance_cost = maintenance_hours * self.cost_inputs.get('maintenance_hourly_rate', 50.0)
        
        # Bond opportunity cost
        bond_amount = 2.0 * validator_count  # Assuming 2 ETH bond per validator
        annual_opportunity_percentage = self.cost_inputs.get('bond_opportunity_cost_annual_percentage', 5.0)
        ...
        # Total costs
        total_cost_usd = hardware_cost + power_cost + bandwidth_cost + maintenance_cost + bond_opportunity_cost_usd
    ...
    def calculate_monthly_revenue(self, validator_count: int = 1) -> Dict[str, float]:
        ...
        if self.performance_history.empty:
            ...
            recent_data = self.performance_history[self.performance_history['timestamp'] >= cutoff_time]
    ...
    def calculate_profitability(self, validator_count: int = 1) -> Dict[str, Any]:
        ...
        setup_cost_usd = self.cost_inputs.get('validator_setup_cost', 500.0) * validator_count
    ...
    def forecast_profitability(self, validator_count: int = 1, months: int = 12) -> List[Dict[str, Any]]:
    ...
    def get_profitability_analysis(self, validator_count: int = 1) -> Dict[str, Any]:
        ...
        if not self.performance_history.empty:
            ...
            daily_data = self.performance_history.set_index('timestamp').resample('D').mean().reset_index()
...
def get_forecast():
    ...
    return jsonify(profitability_calculator.forecast_profitability(validator_count, months))
...
def manage_cost_inputs():
    ...
    if request.method == 'POST':
        try:
            ...
            return jsonify({'status': 'success', 'message': 'Cost inputs updated successfully'})
        except Exception as e:
...
def update_data():
    ...
    return jsonify({'status': 'success', 'message': 'Performance data updated successfully'})
