...
class ObolMetricsCollector:
    ...
    def __init__(self):
        ...
        self.metrics_history_file = os.path.join(METRICS_DATA_DIR, 'obol_metrics_history.json')
    ...
    def collect_metrics(self):
        ...
        charon_metrics_text = self.fetch_prometheus_metrics(self.charon_metrics_endpoint)
        charon_metrics = self.parse_prometheus_metrics(charon_metrics_text)
        
        # Fetch validator metrics
        validator_metrics_text = self.fetch_prometheus_metrics(self.validator_metrics_endpoint)
        ...
        if charon_metrics:
            self.metrics_history['charon'].append({
                'timestamp': timestamp,
                'metrics': charon_metrics
            })
            
            # Keep only last 30 days of data
            if len(self.metrics_history['charon']) > 8640:  # 30 days at 5-minute intervals
        ...
        if validator_metrics:
            ...
            # Keep only last 30 days of data
            if len(self.metrics_history['validator']) > 8640:  # 30 days at 5-minute intervals
                self.metrics_history['validator'] = self.metrics_history['validator'][-8640:]
    ...
    def get_metrics_history(self, days=DEFAULT_HISTORY_DAYS):
        ...
        charon_history = [
            entry for entry in self.metrics_history['charon']
            if entry['timestamp'] >= cutoff_time
        ]
        
        validator_history = [
            entry for entry in self.metrics_history['validator']
            if entry['timestamp'] >= cutoff_time
        ]
        
        return {
            'charon': charon_history,
            'validator': validator_history
        }
    ...
    def get_comprehensive_analysis(self):
        ...
        missed_penalty = (duty_performance['missed_attestations'] * 2) + (duty_performance['missed_blocks'] * 10)
        health_score = max(0, min(100, (attestation_score + consensus_score) / 2 - missed_penalty))
