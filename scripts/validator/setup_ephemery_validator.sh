...
if type log_info &>/dev/null; then
  log_info "Setting up validator password management..."
else
  echo -e "${BLUE}Setting up validator password management...${NC}"
fi

# Check if password manager exists
if [ -f "${SCRIPT_DIR}/validator_key_management/password_manager.sh" ]; then
  if type run_with_error_handling &>/dev/null; then
  ...
  # Fallback to legacy password handling if password manager doesn't exist
