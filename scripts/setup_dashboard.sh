...
start_dashboard() {
    ...
    cd "${DASHBOARD_DIR}" || { echo -e "${RED}Error: Failed to change directory to ${DASHBOARD_DIR}${NC}"; exit 1; }
...
if [ "$install_api" = true ]; then
  ...
  cd "${DASHBOARD_DIR}" || { echo "Error: Failed to change directory to ${DASHBOARD_DIR}"; exit 1; }
