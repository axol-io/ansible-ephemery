...
check_checkpoint_sync_config() {
  ...
  if [[ "${checkpoint_logs}" == *"checkpoint sync"* ]] && [[ "${checkpoint_logs}" != *"checkpoint sync disabled"* ]]; then
    ...
    echo "To enable checkpoint sync, run the ansible/playbooks/fix_checkpoint_sync.yaml playbook."
  fi
...
main() {
  ...
  if [[ "${sync_status}" == *"is_syncing\":true"* ]] && [ "${head_slot}" -eq 0 ]; then
    echo "- Checkpoint sync appears not to be working. Try running the ansible/playbooks/fix_checkpoint_sync.yaml playbook."
  fi
