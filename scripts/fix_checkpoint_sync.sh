...
INVENTORY_FILE="${PROJECT_ROOT}/inventory.yaml"
if [ ! -f "$INVENTORY_FILE" ]; then
    echo -e "${RED}Error: inventory.yaml not found.${NC}"
    echo -e "Please create an inventory file before running this script."
    exit 1
fi

# Check if fix_checkpoint_sync.yaml exists
PLAYBOOK_FILE="${PROJECT_ROOT}/fix_checkpoint_sync.yaml"
if [ ! -f "$PLAYBOOK_FILE" ]; then
    echo -e "${RED}Error: fix_checkpoint_sync.yaml not found.${NC}"
    echo -e "The playbook file should be in the root directory of the project."
    exit 1
fi
